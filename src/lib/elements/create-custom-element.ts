...
/**
 * This is a modified version of Angular Elements `createCustomElement` that
 * can be initialized with just `observedAttributes` instead of an actual
 * ComponentType. This lets the custom element be a shell for the actual
...
export function createCustomElement<P>(
  ...
  class NgElementImpl extends NgElement {
    ...
    upgradedCallback() {
      ...
      if (this.ngElementStrategy.events.length > 0) {
        this.ngElementEventsSubscription = this.ngElementStrategy.events[0].subscribe(callback);
        for (let i = 1; i < this.ngElementStrategy.events.length; i++) {
          this.ngElementEventsSubscription.add(this.ngElementStrategy.events[i].subscribe(callback));
        }
      }
    }
  ...
  inputs.map(({propName}) => propName).forEach(property => {
    Object.defineProperty(NgElementImpl.prototype, property, {
      ...
      set: function(newValue: any) { this.ngElementStrategy.setInputValue(property, newValue, false); },
      configurable: true,
      enumerable: true,
    });
  });
